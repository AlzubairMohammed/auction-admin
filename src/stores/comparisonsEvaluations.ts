...
export const useComparisonsEvaluationsStore = defineStore(
    ...
    actions: {
      ...
      removeComparisonsEvaluation(id) {
        request.delete(this.url, id).then(() => {
          this.comparisonsEvaluations = this.comparisonsEvaluations.filter(
            (r) => r.id !== id
          );
        });
      },
      updateRealestate(realestate) {
        this.realestates = this.realestates.map((r) =>
          r.id === realestate.id ? realestate : r
        );
      },
    },
  }
