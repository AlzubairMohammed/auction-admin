...
const request = {
 ...
 post: (url, payload, options, isReload) => {
  let loader;
  ...
  return new Promise((resolve, reject) => {
   try {
    axios.post(url, payload, options).then((res) => {
     if (res.data.status) {
      ...
      if (url?.includes('search') || url?.includes('paginate')) {
       ...
       responseAlert('success', 'نجاح', res?.data?.msg, isReload);
       loader.hide();
      }
     } else {
      loader.hide();
      responseAlert('error', 'خطاء', res?.data?.msg, isReload);
      reject();
     }
    });
   } catch (error) {
    ...
    if (url.includes('search')) {
     ...
     responseAlert('error', 'خطاء', 'عفوا حدث خطاء ما ', isReload);
    }
   }
   if (url.includes('search')) {
    console.log('search request');
 ...
 delete: (url, id) => {
  return new Promise((resolve, reject) => {
   try {
    Swal.fire({
     title: 'هل انت متأكد ؟',
     text: 'لن تستطيع استعادة العنصر بعد الحذف و سيتم حذف جميع العناصر المرتبطة !',
     icon: 'warning',
