     ...
     <div class="p-4 flex sm:flex-row flex-col w-full sm:items-center gap-4">
      <div class="ltr:mr-3 rtl:ml-3 flex items-center">
       ...
       <div class="relative group flex-1">
        <input type="text" class="form-input peer ltr:!pr-10 rtl:!pl-10" placeholder="Search Task..." v-model="searchTask" @keyup="searchTasks()" />
          ...
          <tr class="group cursor-pointer" :class="{ 'bg-white-light/30 dark:bg-[#1a2941]': task.status === 'complete' }">
           ...
           <td>
            <div @click="viewTask(task)">
             <div class="group-hover:text-primary font-semibold text-base whitespace-nowrap" :class="{ 'line-through': task.status === 'complete' }">
              {{ task.title }}
             </div>
             <div class="text-white-dark overflow-hidden min-w-[300px] line-clamp-1" :class="{ 'line-through': task.status === 'complete' }">
               ...
               >
                ...
                <template #content="{ close }">
                 <ul @click="close()" class="text-sm text-medium">
                  <li>
                   <button type="button" class="w-full text-danger ltr:text-left rtl:text-right" @click="setPriority(task, 'high')">High</button>
                  </li>
                  <li>
                   <button type="button" class="w-full text-primary ltr:text-left rtl:text-right" @click="setPriority(task, 'medium')">Medium</button>
                  </li>
                  <li>
                   <button type="button" class="w-full text-warning ltr:text-left rtl:text-right" @click="setPriority(task, 'low')">Low</button>
                  </li>
                 </ul>
                </template>
               </Popper>
              ...
              <div class="dropdown">
               ...
               >
                ...
                <template #content="{ close }">
                 <ul @click="close()" class="text-sm text-medium">
                  <li>
                   <button type="button" class="w-full text-success" @click="setTag(task, 'team')">Team</button>
                  </li>
                  <li>
                   <button type="button" class="w-full text-info" @click="setTag(task, 'update')">Update</button>
                  </li>
                  <li>
           ...
           <td class="w-1">
            <p class="whitespace-nowrap text-white-dark font-medium" :class="{ 'line-through': task.status === 'complete' }">
             {{ task.date }}
            </p>
           </td>
           <td class="w-1">
            <div class="flex items-center justify-between w-max">
             <div class="ltr:mr-2.5 rtl:ml-2.5 flex-shrink-0">
              ...
              <div v-if="!task.path && task.assignee" class="grid place-content-center h-8 w-8 rounded-full bg-primary text-white text-sm font-semibold">
               {{ task.assignee.charAt(0) + '' + task.assignee.charAt(task.assignee.indexOf(' ') + 1) }}
              </div>
              <div v-if="!task.path && !task.assignee" class="border border-gray-300 dark:border-gray-800 rounded-full grid place-content-center h-8 w-8">
